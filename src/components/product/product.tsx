...
export const Product: React.FC<ProductType> = (props) => {
  ...
  return (
    <div className="flex min-w-[241px] flex-col gap-5 rounded-lg bg-neutral-400 p-5 shadow-m">
      {/* Header */}
      <div className={`flex items-center ${product.discountPercentage ? "justify-between" : "justify-end"}`}>
        {product.discountPercentage && (
          <div className="rounded-lg bg-white px-3 py-1">
            <p className="body-3 font-bold text-primary-400">{product.discountPercentage}%</p>
          </div>
        )}
        ...
        <div className="flex items-center justify-center gap-2">
          ...
          <div className="flex gap-2">
            {props.products?.map((product, index) => {
              return <button key={index} onClick={() => setProduct(props.products![index])} style={{ background: product.color }} className="h-3 w-3 rounded-full bg-white" />;
            })}
          </div>
        </div>
      </div>
      ...
      <div className="flex w-full justify-center">
        <Button onClick={() => props.onBuy(product)} text="Add To Chart" type="default" />
      </div>
    </div>
  );
};
