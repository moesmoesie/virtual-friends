...
export const SelectInput: React.FC<InputSelectProps> = (props) => {
  ...
  return (
    <div
      className={`border-b medium:pt-4 outline-none pt-3 border-dark-purple-200 w-full bg-transparent
            ${props.status === "idle" ? "!border-dark-purple-200" : ""}
            ${props.status === "succes" ? "!border-teal-500" : ""}
            ${props.status === "error" ? "!border-[#FF0000]" : ""}
    `}
    >
      ...
      <div className="flex gap-4 no-scrollbar overflow-x-scroll pb-3 medium:pb-4  flex-nowrap">
        {props.options.map((option, index) => {
          return (
            <button
              ...
              className={`rounded-lg border whitespace-nowrap border-grey-200 body-1 h-8 ${
